...
class ResponseWriter():
    ...
    def write_response(response):
        ResponseWriter.logfile.write('%s, %s, %s, %s, %.9f, %s\n' % (datetime.datetime.now().isoformat(), response.status_code, response.reason, response.request.url, response.elapsed.total_seconds(), response.text))
