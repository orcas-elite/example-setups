...
public abstract class MicroserviceType {
	...
	ResponseEntity<String> responsed;
				String responsedStr;
				try {
					responsed = restTemplate.exchange("http://d:8080/d1", HttpMethod.GET, request, String.class);
					responsedStr = responsed.getBody();
				} catch (Exception e) {
					responsedStr = "Operation d/d1 failed";
				}
	ResponseEntity<String> responsee;
				String responseeStr;
				try {
					responsee = restTemplate.exchange("http://e:8080/e1", HttpMethod.GET, request, String.class);
					responseeStr = responsee.getBody();
				} catch (Exception e) {
					responseeStr = "Operation e/e1 failed";
				}
		String returnStr = "";
	returnStr += responsedStr + "<br>";
	returnStr += responseeStr + "<br>";
		returnStr += "<br>Operation b/b1 executed successfully.";
	...
	ResponseEntity<String> responsee;
		...
		String returnStr = "";
	returnStr += responseeStr + "<br>";
		returnStr += "<br>Operation b/b2 executed successfully.";
		return new ResponseEntity<String>(returnStr, HttpStatus.OK);
	}
}
