...
public abstract class MicroserviceType {
	...
	public ResponseEntity<String> c(@RequestHeader(value="x-request-id", required=false) String xreq,
		...
		restTemplate.exchange("http://d:8080/g", HttpMethod.GET, request, String.class);
		restTemplate.exchange("http://e:8080/h", HttpMethod.GET, request, String.class);
		return new ResponseEntity<String>("Operation c executed successfully.", HttpStatus.OK);
	}
	@RequestMapping(value = "/d", method = GET)
	public ResponseEntity<String> d(@RequestHeader(value="x-request-id", required=false) String xreq,
			@RequestHeader(value="x-b3-traceid", required=false) String xtraceid,
            @RequestHeader(value="x-b3-spanid", required=false) String xspanid,
            @RequestHeader(value="x-b3-parentspanid", required=false) String xparentspanid,
            @RequestHeader(value="x-b3-sampled", required=false) String xsampled,
